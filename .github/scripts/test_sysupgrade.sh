...
run --self_update -r     # self_update is opt-in in this fork
...
# ...and without --self_update the script in /tmp is never even looked at. This
# fork does not self-update by default, so the fall-through the case above
# guards is unreachable on a normal run; assert that rather than assume it.
reset_env
STUB_CURL_RC=7
printf '#!/bin/sh\nscr_version=0.0.0\necho STALE SCRIPT RAN\nexit 42\n' > "$SB/tmp/sysupgrade"
run -r
[ "$RC" -ne 42 ] && ! printf '%s\n' "$OUT" | grep -q 'STALE SCRIPT RAN' \
    && ok "the default (no --self_update) run never consults /tmp/sysupgrade at all" \
    || bad "a default run exec'd the stale script, rc=$RC out='$OUT'"
rm -f "$SB/tmp/sysupgrade"
...
for opt in $(grep -oE '\-\-[a-z_]+' "$SRC" | sort -u); do
    case "$opt" in
        --force_*|--wipe_overlay|--no_reboot|--no_update|--self_update|--no_ramfs|--help|--web|--url|--archive|--kernel|--rootfs|--channel|--build|--list*|--insecure|--connect*|--speed*|--proto*) continue ;;
    esac
